...
- **Need input the caller didn't supply?** `return ctx.requestInput(...)` and read `ctx.inputs` when the handler is re-entered. Never `await` for user input mid-handler. No handler here does — every tool answers from the bundled index in one round.
...
This server is **tool-only** (six `medcode_*` tools, no resources or prompts) and reads a bundled read-only SQLite index — handlers compose synchronous query methods on the `code-index` service.
...
export const checkCodeTool = tool('medcode_check_code', {
  ...
  input: z.object({
    ...
    system: z.enum(SYSTEM_IDS).optional().describe('Force the lookup into this system. Omit to auto-detect.'),
  }),
  output: z.object({
  ...
  // Typed per-tool error contract — ctx.fail is type-checked against these reasons.
  errors: [
    { reason: 'unknown_code', code: JsonRpcErrorCode.NotFound,
...
`parseEnvConfig` maps Zod schema paths → env var names so errors name the variable (`MEDCODE_MAX_RESULTS`) not the path (`maxResults`). Throws `ConfigurationError`, which the framework prints as a clean startup banner. The server is keyless — both vars are optional. (`emptyAsUndefined` treats `""` and unsubstituted MCPB `${...}` placeholders as absent so an empty `MEDCODE_DB_PATH=` falls back to the bundled default.)
...
await createApp({
  ...
  title: 'medical-codes-mcp-server',          // display name — the machine name, NOT Title Case
  sessionMode: 'stateless',                   // HTTP session posture, declared in src/
  tools: [getCodeTool, searchCodesTool, checkCodeTool, mapCodesTool, browseHierarchyTool, listSystemsTool],
  instructions: 'Offline US healthcare code lookup and crosswalk …', // session-level context
  landing: { requireAuth: false, tagline: '…', repoRoot: '…', links: [/* .gov sources */] },
  async setup() {
    await initCodeIndexService();             // open the bundled DB read-only once at startup
  },
  teardown() {
    closeCodeIndexService();                  // release the SQLite file handle on shutdown
  },
});
```
...
`sessionMode` declares the HTTP session posture in `src/` instead of leaving it to a deployment's `MCP_SESSION_MODE`, which still wins whenever it carries a meaningful value (an empty string and an unsubstituted `${…}` placeholder read as unset and fall through to the option). This server declares `stateless`: no handler calls `ctx.requestInput`, so there is no mid-handler prompt a 2025-era stateless client could fail to answer, and `require: 'stateful'` would be wrong here. Stdio is never refused either way.

`teardown(core)` is the `setup()` counterpart — release a watcher, socket, or non-`unref()`'d timer there. It runs after the transport stops and before the logger closes, on every shutdown path, and a signal-triggered shutdown then exits the process explicitly (0, or 1 if a step never settles within the framework's 10 s ceiling). Here it closes the read-only SQLite handle that `setup()` opened.
...
```text
src/
  index.ts                              # createApp() entry point — registers the six tools, opens the index in setup(), closes it in teardown()
  config/
    server-config.ts                    # Server-specific env vars (MEDCODE_DB_PATH, MEDCODE_MAX_RESULTS)
  services/
    code-index/
      code-index-service.ts             # Read-only SQLite handle + typed query methods (init/accessor pattern)
      detect.ts                         # Code-shape auto-detection (which system a raw code belongs to)
      schema.ts                         # Shared DDL + code-shape derivation (imported by service AND build script)
      types.ts                          # Domain types (SystemId, CodeRow, MapDirection, …)
  mcp-server/
    tools/definitions/
      get-code.tool.ts                  # medcode_get_code
      search-codes.tool.ts              # medcode_search_codes
      check-code.tool.ts                # medcode_check_code
      map-codes.tool.ts                 # medcode_map_codes
      browse-hierarchy.tool.ts          # medcode_browse_hierarchy
      list-systems.tool.ts              # medcode_list_systems
      _render.ts                        # Shared format() helpers (renderCodeLine / renderCodeBlock)
scripts/
...
No `resources/` or `prompts/` — the server is tool-only by design (every datum is reachable through the tool surface; tool-only clients can't see resources).
...
|:-----|:-----------|:--------|
| Files | kebab-case with suffix | `check-code.tool.ts` |
| Tool/resource/prompt names | snake_case | `medcode_check_code` |
| Directories | kebab-case | `src/services/code-index/` |
...
| `api-mirror` | MirrorService: persistent self-refreshing local mirror (embedded SQLite + FTS5) of a bulk upstream dataset — Tier 3 opt-in; not used by this server |
...
- [ ] `ctx.log` for logging (this server has no tenant state — the code index is a read-only global)
