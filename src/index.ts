...
await createApp({
  ...
  instructions:
    'Offline US healthcare code lookup and crosswalk over a bundled SQLite index — keyless, no rate limit, deterministic. Bundled systems: ICD-10-CM (diagnoses), ICD-10-PCS (inpatient procedures), HCPCS Level II (supplies/drugs/services), and RxNorm drugs (the current normalized set). Decode a code with medcode_get_code (the 80% entry point; accepts a batch, auto-detects the system per code, and decodes an NDC directly to its RxNorm product). Go description → code with medcode_search_codes. Validate billability with medcode_check_code — a non-billable or terminated code is a successful result with a whyNot, not an error. RxNorm has no billing concept: its concepts carry billable: null and check as valid, with no billing verdict. Walk the hierarchy with medcode_browse_hierarchy (prefix for ICD-10-CM/HCPCS, axis-based for ICD-10-PCS). Crosswalk with medcode_map_codes — code parents/children, the RxNorm drug directions (drug name → RXCUI, NDC ↔ RXCUI, RXCUI → ingredients/brands), and the RxClass drug-class directions (RXCUI → classes: pharmacologic class, mechanism, physiologic effect, indications and contraindications, VA class, DEA schedule, vaccine code; class ID → member RXCUIs). A class hit whose relation starts with ci_ is a contraindication, not an indication. DEA schedules are recorded only on drug products, and VA classes almost only there, so map a product rather than an ingredient for those. Confirm which releases and RxClass sources are active with medcode_list_systems. ICD-10-CM/PCS are the US clinical modifications, not the ICD-10/ICD-11 base.',
  landing: {
    requireAuth: false,
    tagline:
