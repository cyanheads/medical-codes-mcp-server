...
2. Parses the format: ICD-10-CM/PCS order files = fixed-width slice by documented column positions; HCPCS `ANWEB.txt` = fixed-width (320 chars/row — not delimited, despite the "columns" label in CMS docs); RxNorm = the RxNav JSON cache written by `scripts/ingest/fetch-rxnav.ts`.
...
| RxNorm | get-by-RXCUI, search-by-drug-name, name→RXCUI, NDC↔RXCUI, RXCUI→ingredients/brands | get / search / map |
...
Verified against primary `.gov` sources (June 2026). ICD-10-CM, ICD-10-PCS, and HCPCS are static bulk downloads; RxNorm is fetched over the keyless RxNav API at build time. None needs a key, and none is called at runtime:
...
| `medcode_map_codes` | `field_not_applicable` | `InvalidParams` | a `system`, `limit`, or `cursor` the direction does not use — checked after `direction_unavailable` (re-calling without the field would only reach that error) and before the cursor is decoded or any lookup runs | Drop the named fields and re-call: `system` steers only parents/children (the drug directions accept only `RXNORM`, a no-op); `limit`/`cursor` apply only to children, name_to_rxcui, and rxcui_to_ndc. |
| `medcode_map_codes` | `no_mapping` | `NotFound` | the source value did not resolve to any bundled code | Check the code, or decode it with `medcode_get_code` first. A resolvable source with no edge in the direction — or a `cursor` past the last page — is an empty result with a notice, not this error. These misses throw their own hint instead: a code-system name in `from` (`ICD10CM`, `HCPCS Level II`, …) is told where the code and the system go; a `parents`/`children` source an explicit `system` missed names every other bundled system holding it and recovers to a re-call with that `system`, as `medcode_check_code` and `medcode_get_code` name it; an `ndc_to_rxcui` miss names the accepted FDA segment configurations for a malformed spelling, or says no bundled drug maps to a well-formed NDC, and recovers to the package label or `name_to_rxcui` rather than to `medcode_get_code`; on `parents`/`children`/`rxcui_to_*`, an NDC that `medcode_get_code` would decode is named as an NDC and sent to `ndc_to_rxcui`, a well-formed NDC no bundled drug maps to (hyphenated, or bare 10/11 digits) gets the `ndc_to_rxcui` miss wording and recovery, and any other bare integer no bundled system holds is named as a possible CPT / HCPCS Level I code and sent to `medcode_search_codes`. |
