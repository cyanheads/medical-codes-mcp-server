...
- `rxclass_edge` — `rxcui TEXT, class_type TEXT, class_id TEXT, source TEXT, relation TEXT`, PK over all five — one row per bundled RXCUI × class × asserting source × relation, exactly as RxClass attaches it (mostly at `IN`/`PIN`/`MIN`; VA classes and DEA schedules at the product). Nothing inherited is stored: `rxcui_to_classes` reaches a product's ingredient classes through `rxnorm_rel has_ingredient` at query time. `relation` is lowercased to RxClass's own `relas` vocabulary.
