...
| `medcode_map_codes` | `no_mapping` | `NotFound` | the source value did not resolve to any bundled code | Check the code, or decode it with `medcode_get_code` first. A resolvable source with no edge in the direction — or a `cursor` past the last page — is an empty result with a notice, not this error. |
