...
| `medcode_map_codes` | `no_mapping` | `NotFound` | the source value did not resolve to any bundled code | Check the code, or decode it with `medcode_get_code` first. A resolvable source with no edge in the direction — or a `cursor` past the last page — is an empty result with a notice, not this error. These misses throw their own hint instead: a code-system name in `from` (`ICD10CM`, `HCPCS Level II`, …) is told where the code and the system go; an `ndc_to_rxcui` miss names the accepted FDA segment configurations for a malformed spelling, or says no bundled drug maps to a well-formed NDC, and recovers to the package label or `name_to_rxcui` rather than to `medcode_get_code`; on `parents`/`children`/`rxcui_to_*`, an NDC that `medcode_get_code` would decode is named as an NDC and sent to `ndc_to_rxcui`, and any other bare integer no bundled system holds is named as a possible CPT / HCPCS Level I code and sent to `medcode_search_codes`. |
