...
| `medcode_search_codes` | Find codes whose official descriptions match a described concept, via full-text search over the bundled index. Filter by system, billable status, and chapter. Use when you have a clinical description ("type 2 diabetes with diabetic neuropathy") and need the code, not the other way round. | `query`, `system?`, `billableOnly?`, `chapter?`, `limit?`, `cursor?` | `readOnlyHint`, `idempotentHint`, `openWorldHint:false` |
| `medcode_check_code` | Validate whether a code exists, is current, and is billable in the active release. Returns status with a why-not for invalid or non-billable codes ("valid ICD-10-CM category but not billable — requires more specific child code"); a current RxNorm concept is `valid` with no billing verdict, since RxNorm has no billing concept. The caller-facing detail a coder needs before submitting a claim. | `code`, `system?` | `readOnlyHint`, `idempotentHint`, `openWorldHint:false` |
| `medcode_map_codes` | Crosswalk a code or drug across systems and within a hierarchy: drug name → RxNorm RXCUI, NDC ↔ RXCUI, RXCUI → ingredients/brands, RXCUI ↔ RxClass drug class, and any code → its parent/child codes. The relational bridge between the bundled systems and a composition point with `openfda` (NDC/labels). | `from` (value), `direction` (enum), `system?` (`parents`/`children`; drug and class directions accept only `RXNORM`), `classType?` (`rxcui_to_classes`, `class_to_rxcuis` only), `limit?` / `cursor?` (`children`, `name_to_rxcui`, `rxcui_to_ndc`, `rxcui_to_classes`, `class_to_rxcuis` only) | `readOnlyHint`, `idempotentHint`, `openWorldHint:false` |
| `medcode_browse_hierarchy` | Walk a code system's hierarchy (chapter → section → category → code) for discovery without a search term. Returns the children of a node, or the top-level chapters when no node is given. Lets an agent orient in an unfamiliar system or enumerate a category's specific codes. | `system`, `node?`, `limit?`, `cursor?` | `readOnlyHint`, `idempotentHint`, `openWorldHint:false` |
| `medcode_list_systems` | List the bundled code systems with their release identifiers, effective dates, and code counts, plus the RxClass class layer with each source's version, class and edge counts, and fetch date. Lets a caller confirm which ICD-10 fiscal year, HCPCS quarter, RxNorm snapshot, and RxClass sources are active before acting on results. Cheap orientation / provenance call. | _(none)_ | `readOnlyHint`, `idempotentHint`, `openWorldHint:false` |
...
| RxNorm (current normalized set, via RxNav) | NLM — public domain | Normalized drug names: ingredients, brands, clinical/branded drugs and packs, with RXCUIs, NDC links, and ingredient/brand edges (55,867 concepts in the June 2026 build) | Monthly |
| RxClass drug classes (via the RxClass API) — a class layer over the RxNorm concepts, not a code system | US government sources only: VA MED-RT, FDA SPL, Federal Medication Terminologies, VA National Formulary, RxNorm DEA schedules, CDC CVX | 20,707 classes and 86,837 drug–class edges: pharmacologic class, mechanism, physiologic effect, pharmacokinetics, therapeutic category, chemical structure, disease relations (indications and contraindications), VA class, DEA schedule, vaccine code | Per source — each source's RxClass version is recorded and reported |

**Deliberately excluded:** CPT (AMA copyright), SNOMED CT / LOINC (UMLS-license-gated), and the ATC, ATCPROD, SNOMEDCT, and DAILYMED RxClass sources. The server's value is the union of the *freely-redistributable* code sets — bundling a license-gated set would poison the package. See Design Decisions §1.
...
4. **Crosswalk** across systems (drug name → RXCUI, NDC ↔ RXCUI, RXCUI → ingredients, RXCUI ↔ drug class) and within a hierarchy (code → parents/children) → `medcode_map_codes`
...
- **Code-shape auto-detection.** ICD-10-CM (`A00`–`Z99` letter + 2 digits + optional `.` + up to 4 more alphanumerics), ICD-10-PCS (7-char alphanumeric, no letters `I`/`O`), HCPCS Level II (1 letter `A`–`V` + 4 digits), RXCUI (pure integer) have distinct shapes. `medcode_get_code`/`medcode_check_code`/`medcode_map_codes` route by pattern; an explicit `system` overrides when a value is ambiguous (e.g. a short string that could be a partial ICD-10-CM category or an HCPCS code). The patterns describe *complete* codes, so they narrow rather than decide: DB membership is the arbiter, and when no shaped candidate is a member the search widens to every bundled system. That keeps the header rows the index materializes — the HCPCS letter buckets (`J`) and the 3-character ICD-10-PCS table rows (`001`, `00B`) — decodable without an explicit `system`, since browse and search both hand them back. The 60 PCS table rows that are also ICD-10-CM categories (`B00`) are the one case where the shape pass picks between two real members: only the ICD-10-CM pattern admits a 3-character letter+2-digit value, so they resolve there and reaching the PCS row needs an explicit `system`. Widening them instead would turn 60 working lookups into `ambiguous_system`, so the resolution is left exactly as it is and the excluded member is *disclosed* on the success path — see §9.
...
- **Licensing posture.** Only freely-redistributable public-domain sets are bundled. RxNorm is the public-domain normalized vocabulary, fetched at build time over the keyless RxNav API — never the full monthly RRF release, which would drag in UMLS-licensed source vocabularies and break redistribution. See Design Decisions §1.
...
- Expose typed query methods the tools compose: `getByCode(code, system?)`, `searchFts(query, filters)`, `checkCode(code, system?)`, `mapCode(from, direction, system?, page?, classType?)`, `browse(system, node?, page)`, `listSystems()`, `classLayer()`, plus the `hasRxNorm()` / `hasClassLayer()` gates behind `direction_unavailable`.
...
- `build_meta` — `system TEXT, release_id TEXT, effective_start TEXT, effective_end TEXT, code_count INTEGER, source_url TEXT, built_at TEXT` — one row per system, the provenance `medcode_list_systems` returns. For RxNorm, which has no release label, `built_at` is the RxNav snapshot date: the time `fetch-rxnav.ts` completed its cache, recorded in `.sources/rxnav/meta.json` and read by the build, so a rebuild from the same cache reports the same date. The other systems carry the build time.
- `rxclass_class` — `class_type TEXT, class_id TEXT, class_name TEXT`, PK `(class_type, class_id)` — every class node of the bundled class types, hierarchy nodes with no direct member included. A class ID is unique within its type only (a few MeSH IDs are both `CHEM` and `DISEASE`).
- `rxclass_edge` — `rxcui TEXT, class_type TEXT, class_id TEXT, source TEXT, relation TEXT`, PK over all five — one row per bundled RXCUI × class × asserting source × relation, exactly as RxClass attaches it (mostly at `IN`/`PIN`/`MIN`; DEA schedules only at the product, VA classes almost only there — 46 of the 19,704 VA edges sit on ingredient concepts). Nothing inherited is stored: `rxcui_to_classes` reaches a product's ingredient classes through `rxnorm_rel has_ingredient` at query time. `relation` is lowercased to RxClass's own `relas` vocabulary.
- `rxclass_source` — `source TEXT, version TEXT, class_count INTEGER, edge_count INTEGER, fetched_at TEXT` — per bundled source, the provenance `medcode_list_systems` returns under `classLayer`. `version` is null where RxClass publishes none (CDC); `fetched_at` is the time `fetch-rxclass.ts` completed its cache, recorded in `.sources/rxclass/meta.json`, never the build time. The class layer is not a system, so it has no `build_meta` row and no `SYSTEM_IDS` entry. An index built before the layer has none of these three tables, and one built from sources with no RxClass cache has them empty; neither carries the layer, so the class directions report `direction_unavailable` on both and `classLayer` is null.
...
2. Parses the format: ICD-10-CM/PCS order files = fixed-width slice by documented column positions; HCPCS `ANWEB.txt` = fixed-width (320 chars/row — not delimited, despite the "columns" label in CMS docs); RxNorm = the RxNav JSON cache written by `scripts/ingest/fetch-rxnav.ts`; RxClass = the JSON cache written by `scripts/ingest/fetch-rxclass.ts`.
3. Normalizes into the `codes`/`rxnorm_rel`/`ndc_map` rows, derives `parent`/`chapter`/`header` from code structure (CM/HCPCS only; PCS `parent` stays NULL and hierarchy data populates `pcs_axes` from the tabular XML), writes `build_meta`; then keys the RxClass edges on the bundled RxNorm concepts, dropping the excluded sources and any edge to an RXCUI outside the bundle, and writes `rxclass_class` / `rxclass_edge` / `rxclass_source`.
...
| `MEDCODE_MAX_RESULTS` | no | Default page size when a call sends no `limit` — `medcode_search_codes`, `medcode_browse_hierarchy`, and the paginated `medcode_map_codes` directions — and the number of children `medcode_get_code` attaches with `includeHierarchy` (default 50, hard ceiling 200). Bounds context budget. |
...
6. **RxNorm (phase 2)** — add the RxNorm parse (`codes` + `rxnorm_rel` + `ndc_map`, from the RxNav cache — see the 0.2.0 update under Design Decisions §4), then `medcode_map_codes`. This is the only system with relational crosswalk complexity; landing it last keeps v1 unambiguously public-domain (see Design Decisions §4).
...
| RxNorm | get-by-RXCUI, search-by-drug-name, name→RXCUI, NDC↔RXCUI, RXCUI→ingredients/brands, RXCUI↔RxClass class | get / search / map |
...
- **RxNorm normalized vocabulary** (NLM) — the public-domain SAB=RXNORM layer (normalized names, RXCUIs, NDC links, ingredient/brand edges), fetched at build time over the keyless RxNav API, which serves only that layer. The original plan used the RRF *Prescribable Content* subset; NLM has since moved the RRF bulk files behind UMLS login (see the 0.2.0 update under §4). **This is the only nuance in the whole server**, and it's a sourcing detail, not a blocker.
...
- **RxClass drug classes** — per source, since RxClass aggregates sources with different owners. Bundled, all US government works: `MEDRT` (VA MED-RT), `FDASPL` (FDA established pharmacologic classes from structured product labels), `FMTSME` (Federal Medication Terminologies therapeutic categories), `VA` (VA National Formulary classes), `RXNORM` (DEA controlled-substance schedules as RxNorm records them — covered by the RxNorm public-domain terms), and `CDC` (CVX vaccine codes). Excluded: `ATC` and `ATCPROD`, whose WHO terms state "Copying and distribution for commercial purposes is not allowed" — incompatible with an Apache-2.0 package; `SNOMEDCT`, under the SNOMED CT Affiliate license; and `DAILYMED`, which is redistributable but repeats FDASPL (8,694 of its 8,710 edges) and publishes no version to record. The MeSH IDs that name the `CHEM` and `DISEASE` classes are NLM-provided identifiers, not SNOMED content. The build filters by source, so a new RxClass source is never bundled without being added to `RXCLASS_SOURCES` deliberately.

The hard build-time rule: *never* pull the full monthly RxNorm release. It bundles UMLS-licensed source vocabularies; shipping it would silently convert a clean public-domain package into a license-violating one. The build acquires RxNorm only through RxNav (`scripts/ingest/fetch-rxnav.ts`), and the fetcher's header records why, so a future maintainer doesn't "upgrade" to the full release.

### 2. Data sources — exact files, formats, and parse points

Verified against primary `.gov` sources (June 2026; RxClass September 2026). ICD-10-CM, ICD-10-PCS, and HCPCS are static bulk downloads; RxNorm and RxClass are fetched over the keyless RxNav APIs at build time. None needs a key, and none is called at runtime:
...
| RxNorm | NLM RxNav REST API (`rxnav.nlm.nih.gov/REST/`), keyless — fetched at build time by `scripts/ingest/fetch-rxnav.ts` into `.sources/rxnav/`, never at runtime | `allconcepts.json?tty=…` (the concept set), then per drug product `ndcs.json` and `related.json?tty=IN+PIN+MIN+BN` | JSON. `parseRxNav()` turns the cache into `codes(RXNORM)` rows, `ndc_map` (11-digit NDC ↔ RXCUI), and `rxnorm_rel` (ingredient/brand edges). |
| RxClass | NLM RxClass REST API (`rxnav.nlm.nih.gov/REST/rxclass/`), keyless — fetched at build time by `scripts/ingest/fetch-rxclass.ts` into `.sources/rxclass/`, after the RxNav cache it keys on, never at runtime | `version/<SRC>.json` per bundled source (the documented `version/relaSource.json?relaSource=` form returns 404), `allClasses.json?classTypes=<T>` per class type, then `class/byRxcui.json?rxcui=<id>` per bundled `IN`/`MIN` — RxClass answers an ingredient with its own edges and those of its precise ingredients, multi-ingredients, and products | JSON. `parseRxClass()` keeps the bundled sources and bundled RXCUIs and lowercases each rela (`byRxcui` spells VA's `has_VAClass`; RxClass's `relas` vocabulary spells `has_vaclass`), producing `rxclass_class`, `rxclass_edge`, and `rxclass_source`. |
...
The corpus is **tiny and changes on a slow, scheduled cadence**: on the order of ~243k code rows total (ICD-10-CM ~98k rows / ~75k billable, ICD-10-PCS ~80k, HCPCS ~9k rows, RxNorm ~56k concepts, plus the RxNorm edge and NDC tables; exact counts land in `build_meta` at build time), updated annually (ICD-10), quarterly (HCPCS), monthly (RxNorm) — never intra-day, never per-request. The design skill's backend-by-corpus-size guidance maps three ways, and this corpus sits at the small end:
...
| External store (≳ 10⁸) | Irrelevant at ~243k rows. |
...
- `medcode_get_code` returns, per code: resolved `system`, `code` (display form with dots re-inserted), `description` (long), `shortDescription`, `billable`, `header`, `chapter`, and (when `includeHierarchy`) `parent` + immediate `children`. The agent can chain straight into a `medcode_map_codes` parents/children walk (the drug directions take no `system` but `RXNORM`) or surface a billability warning without a second call.
- A field the system's release does not carry is `null`, never the placeholder the `codes` row stores for it: RxNorm has no billing concept (`billable: null`, rendered `billable: n/a`) and publishes a single name (`shortDescription: null`), and its term type (`IN`, `SCD`, `SBD`, …) is carried in `chapter`, where the search `chapter` filter reads it. One per-system record (`SYSTEM_TRAITS` in `types.ts`) drives both, so the stored placeholders stay and the index is not rebuilt. The same record keeps text matching off the placeholder: `medcode_search_codes` and the `name_to_rxcui` crosswalk read only `long_desc` for a system without a short description, at query time, so a query equal to or inside a term type (`SBD`, `in`) never returns every concept of that type.
- `medcode_check_code` returns a discriminated status (`valid_billable` | `valid_not_billable` | `valid_header` | `valid` | `terminated` | `unknown`) plus a `whyNot` string for the non-billable/terminated cases — the recovery instruction a coder acts on. `valid` is a current code in a system with no billing concept (RxNorm), with `billable: null` and no `whyNot`. An `unknown` value that is a National Drug Code says so and recovers to `medcode_get_code` / `medcode_map_codes` `ndc_to_rxcui` — NDC is a package identifier the server decodes, not a code `check_code` has a verdict for.
- `medcode_search_codes` discloses truncation: `ctx.enrich.truncated({ shown, cap })` when the result hits the cap, and echoes the parsed query + active filters via `ctx.enrich` so both client surfaces see what was searched. Empty results return an explicit notice (try broader terms / different system), not a bare empty array — or, for `billableOnly` on RxNorm, a notice naming that filter as the cause.
...
- The class directions put the RxClass layer on that same surface rather than a separate tool: both lookups are identifier crosswalks with `source` provenance, and `classType` follows the per-direction field rule of `field_not_applicable`. A class hit carries `classType`, `source` (the RxClass source asserting it), and `relation` — one hit per class × source × relation — and `format()` renders all three, spelling a `ci_` relation out as a contraindication so a text-only reader never takes it for an indication. `rxcui_to_classes` reaches a drug product's ingredient classes through `has_ingredient` and names the ingredient in `via`; the product's own edge wins, then an `IN` over its `PIN`, then the lowest RXCUI, and nothing is inherited downward. RxClass records DEA schedules (and all but a handful of VA classes) on drug products, so an ingredient's empty `SCHEDULE` or `VA` result carries a notice saying where those classes live — without it, "oxycodone has no SCHEDULE class" reads as "oxycodone is unscheduled". Inheriting them downward instead would be wrong: an ingredient's products can carry different schedules (codeine's single-ingredient and combination products differ). `class_to_rxcuis` returns direct members with their RxNorm name and term type, and reads a one-digit CVX code as its zero-padded class ID (`3` → `03`). Both paginate: one RXCUI reaches 178 classes and one class 3,181 edges. `medcode_list_systems` reports the layer under `classLayer`, apart from `systems`, because it is not a code system the other tools' `system` input accepts.
...
- A code string that resolves in one system while existing in another carries `alsoInSystems` on the successful result of `medcode_get_code` (per `found[]` entry), `medcode_check_code`, and `medcode_map_codes` (hierarchy directions) — the 60 ICD-10-CM/ICD-10-PCS collisions above. It is a disclosure on an otherwise unchanged answer, not a second verdict: the agent reads which other system holds the same string and re-calls with `system` if it meant that one. Structured because a batch decode needs the attribution per code, and rendered in `format()` because a `content[]`-only client would otherwise be handed one meaning as if it were the only one.
...
|:-----|:-------|:-----|:-----|:-----------------------------|
| `medcode_get_code` | `no_codes_found` | `NotFound` | none of the requested codes resolved — in any bundled system, or in the explicit `system` when one is given | Verify code formatting, or call `medcode_search_codes` with a description to find the right code. Under an explicit `system`, the message names that system and any other bundled system holding a requested code. It also names any bare-integer inputs as possible CPT / HCPCS Level I codes, which are out of scope, any well-formed NDC no bundled drug maps to (hyphenated, or bare 10/11 digits, which are never worded as CPT), and any NDC an explicit `system` kept from the NDC decode; the recovery stays, since a batch can fail for mixed causes. |
| `medcode_check_code` | `unknown_code` | `NotFound` | the code does not exist in the named/detected system | Check the code, or search by description with `medcode_search_codes`. For a value that is an NDC — decoded, or well-formed with no bundled drug mapping to it, bare 10/11 digits included — the message says so and the thrown hint instead names `medcode_get_code` and `medcode_map_codes` `ndc_to_rxcui`. Under an explicit `system`, the message names the other bundled system that holds the code, and the CPT / HCPCS Level I out-of-scope sentence fires only for a bare integer no bundled system holds. |
| `medcode_check_code` | `ambiguous_system` | `InvalidParams` | the code is present in more than one bundled system and no `system` was given | Re-call with an explicit `system` to disambiguate. |
| `medcode_map_codes` | `field_not_applicable` | `InvalidParams` | a `system`, `classType`, `limit`, or `cursor` the direction does not use — checked after `direction_unavailable` (re-calling without the field would only reach that error) and before the cursor is decoded or any lookup runs | Drop the named fields and re-call: `system` steers only parents/children (the drug and class directions accept only `RXNORM`, a no-op); `classType` applies only to rxcui_to_classes and class_to_rxcuis; `limit`/`cursor` apply only to children, name_to_rxcui, rxcui_to_ndc, rxcui_to_classes, and class_to_rxcuis. A blank `classType` is an argument rejection like any value outside the enum; only a blank `cursor` counts as omitted. |
| `medcode_map_codes` | `no_mapping` | `NotFound` | the source value did not resolve to any bundled code, drug, or class | Check the code, or decode it with `medcode_get_code` first. A resolvable source with no edge in the direction — or a `cursor` past the last page — is an empty result with a notice, not this error. These misses throw their own hint instead: a code-system name in `from` (`ICD10CM`, `HCPCS Level II`, …) is told where the code and the system go on `parents`/`children`, and on a drug direction the one input that direction takes (a drug name, an NDC, or an RXCUI with the two directions that produce one); a `parents`/`children` source an explicit `system` missed names every other bundled system holding it and recovers to a re-call with that `system`, as `medcode_check_code` and `medcode_get_code` name it; an `ndc_to_rxcui` miss names the accepted FDA segment configurations for a malformed spelling, or says no bundled drug maps to a well-formed NDC, and recovers to the package label or `name_to_rxcui` rather than to `medcode_get_code`; on `parents`/`children`/`rxcui_to_*`, an NDC that `medcode_get_code` would decode is named as an NDC and sent to `ndc_to_rxcui`, a well-formed NDC no bundled drug maps to (hyphenated, or bare 10/11 digits) gets the `ndc_to_rxcui` miss wording and recovery, and any other bare integer no bundled system holds is named as a possible CPT / HCPCS Level I code and sent to `medcode_search_codes`. On `rxcui_to_classes` a class ID is named as one and sent to `class_to_rxcuis`, ahead of the bare-integer test (CVX class IDs are bare integers). A `class_to_rxcuis` miss is worded for a class ID, never with the CPT sentence: a code-system name, a class type (`EPC`), or a bundled RXCUI sent in its place is named as such, and anything else is an unbundled class ID, recovered to the class IDs `rxcui_to_classes` returns and a note that ATC, SNOMED CT, and DailyMed classes are not bundled. |
| `medcode_map_codes` | `ambiguous_system` | `InvalidParams` | the source code is present in more than one system and no `system` was given | Re-call with an explicit `system` to disambiguate. |
| `medcode_map_codes` | `direction_unavailable` | `InvalidParams` | a drug-crosswalk direction requested but the build carries no RxNorm tables, or a class direction requested but it carries no RxClass class layer (an index built before the layer, or from sources with no RxClass cache, whose class tables are empty) | Use a hierarchy direction (parents/children), or rebuild the index with RxNorm bundled (the shipped default). The class case throws its own hint: use another direction, or unset `MEDCODE_DB_PATH` to use the shipped index, which carries the layer. |
...
`medcode_get_code` declares only `no_codes_found`, because it is array-in / partial-success-out: an unresolvable or cross-system-ambiguous code lands in `notFound[]` with its own reason (and `candidateSystems` when ambiguous) rather than failing the batch. The typed error fires only when *every* requested code fails.
...
- **RxNorm coverage is the current normalized set, not the full release.** Obsolete and retired concepts and the UMLS-licensed source vocabularies in the full release are absent. This is the redistribution tradeoff, surfaced in `medcode_list_systems` (release_id names the set).
- **RxClass: direct edges, no hierarchy, no ATC.** `class_to_rxcuis` returns direct members only, and 13,595 of the 20,707 bundled classes have none — hierarchy nodes whose drugs attach to their subclasses (`N0000193873` "Diuretic"); there is no subclass walk, class-hierarchy browse, or class-name search. ATC/ATCPROD, SNOMED CT, and DailyMed classes are excluded (§1), and drug–drug interactions are not in the layer. An edge to an RXCUI outside the bundled normalized set (a retired concept, a dose-form group) is dropped at build time.
- **One `via` per inherited class.** A product whose ingredients share a class gets one hit for it, naming one ingredient in `via` (an `IN` over its `PIN`, then the lowest RXCUI). The other ingredients that carry the same class are not named: in the shipped index 5,127 products have 23,326 such edges shared by two or more `IN`s. Map each ingredient with `rxcui_to_classes` to see which carry it.
